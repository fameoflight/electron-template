...
export class ResolverGenerator {
  ...
  generate(): {
    ...
    if (!this.shouldGenerateResolvers()) {
      ...
      console.log(`Skipping resolver generation for entity "${this.entity.name}" due to graphql configuration.`);
      return {
        resolverPath: '',
  ...
  private generateResolverExtension(): { path: string; created: boolean } {
    ...
    if (!this.templateManager.hasTemplate('resolver-extension')) {
      if (!this.options.dryRun) {
        output.warning(`  ⚠️  Template 'resolver-extension.hbs' not found, skipping resolver extension`);
      }
