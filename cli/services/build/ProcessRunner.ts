      ...
      try {
        ...
        if (isTimeout && fallbackCommand) {
          ...
          try {
            ...
            console.error(`❌ Fallback command also failed: ${errorMessage}`);
          }
        }

        if (attempt <= retries) {
          const errorMessage = error instanceof Error ? error.message : String(error);
          console.warn(`⚠️  ${name} failed (attempt ${attempt}/${retries + 1}): ${errorMessage}`);
          continue;
        }

        // All attempts failed
        const errorMessage = error instanceof Error ? error.message : String(error);
        console.error(`❌ ${name} failed after ${attempt} attempts: ${errorMessage}`);
        throw error;
      }
    }
  }
  ...
  private async runWithTimeout(
    ...
    return new Promise((resolve, reject) => {
      ...
      proc.on('error', (error) => {
        ...
        const errorMessage = error instanceof Error ? error.message : String(error);
        console.error(`${name} process error: ${errorMessage}`);
        reject(error);
      });
