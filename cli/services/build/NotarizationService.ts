...
export class NotarizationService {
    ...
    for (const configFile of configFiles) {
      if (fs.existsSync(configFile)) {
        try {
          ...
          console.log(`✅ Loaded notarization config from ${configFile}`);
          console.log('   Apple ID:', this.maskSensitiveData(this.config.appleId));
          console.log('   Team ID:', this.config.teamId);
          console.log('   Bundle ID:', this.config.bundleId);
          return this.config;
        } catch (error) {
          const errorMessage = error instanceof Error ? error.message : String(error);
          console.warn(`⚠️  Could not parse ${configFile}: ${errorMessage}`);
        }
      }
    }

    console.log('ℹ️  No notarization configuration found - build will be unsigned');
    return this.config;
  }
  ...
  async notarize(): Promise<string[]> {
    ...
    for (const dmgFile of dmgFiles) {
      ...
      try {
        ...
        console.log(`✅ ${dmgFile} uploaded for notarization`);
        console.log('   Check your email for notarization status');
      } catch (error) {
        const errorMessage = error instanceof Error ? error.message : String(error);
        console.error(`❌ Failed to notarize ${dmgFile}: ${errorMessage}`);
        throw error;
      }
    }
  ...
  private async notarizeSingleFile(dmgPath: string): Promise<void> {
    console.log(`📤 Uploading ${path.basename(dmgPath)} for notarization...`);
