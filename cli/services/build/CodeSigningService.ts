...
export class CodeSigningService {
  ...
  async detectSigningIdentity(): Promise<SigningIdentity> {
    try {
      ...
      console.warn('⚠️  Could not check for code signing identities:', error);
      return { hash: null, status: 'none' };
    }
  }
  ...
  async applyHardenedRuntime(opts: CodeSigningOptions = {}): Promise<string[]> {
    ...
    if (appFiles.length === 0) {
      console.warn('⚠️  No .app files found for hardening');
      return [];
    }
    ...
    for (const appPath of appFiles) {
      try {
        ...
        console.log(`✅ Hardened runtime applied to ${path.basename(appPath)}`);
        hardenedApps.push(appPath);
      } catch (error) {
        const errorMessage = error instanceof Error ? error.message : String(error);
        console.warn(`⚠️  Could not apply hardened runtime to ${path.basename(appPath)}: ${errorMessage}`);
      }
    }
